...
test("server-renders the streamlined English portfolio", async () => {
  ...
  assert.match(html, /Verba evidence convergence system map/);
  assert.match(html, /15 historical solution PDFs/);
  assert.match(html, /REUSABLE EVIDENCE/);
  assert.match(html, /Structured Solution Draft/);
  assert.match(html, /Sources &amp; pages/);
...
test("keeps bilingual content and hosted assets wired correctly", async () => {
  ...
  assert.match(content, /历史方案只需索引一次/);
  assert.match(content, /15 份历史方案 PDF/);
  assert.match(content, /复用历史证据/);
  assert.match(content, /结构化方案初稿/);
  ...
  assert.match(media, /verba-prepare-track/);
  assert.match(media, /verba-run-track/);
  assert.match(media, /verba-evidence-feed/);
  assert.match(media, /verba-output-sections/);
...
test("keeps the demo dominant on desktop and stacks media on phones", async () => {
  ...
  assert.match(css, /\.verba-prepare-track\s*\{[^}]*grid-template-columns:\s*repeat\(12/s);
  assert.match(css, /\.verba-run-track\s*\{[^}]*grid-template-columns:\s*repeat\(12/s);
  assert.match(css, /\.verba-run-node\.is-retrieval\s*\{[^}]*grid-column:\s*4\s*\/\s*8/s);
  assert.match(css, /\.verba-evidence-feed\s*\{[^}]*left:\s*45\.8%/s);
  assert.match(css, /@media \(max-width:\s*980px\)[\s\S]*?\.verba-run-track\s*\{[^}]*grid-template-columns:\s*1fr/s);
  assert.match(css, /@media \(max-width:\s*700px\)[\s\S]*?\.verba-prepare-track\s*\{[^}]*grid-template-columns:\s*1fr/s);
});
