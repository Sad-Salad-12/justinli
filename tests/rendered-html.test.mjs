...
test("server-renders the streamlined English portfolio", async () => {
  ...
  assert.match(html, /Verba source-linked solution cycle/);
  assert.match(html, /15 case PDFs/);
  assert.match(html, /EVIDENCE-TO-SOLUTION LOOP/);
...
test("keeps bilingual content and hosted assets wired correctly", async () => {
  ...
  assert.match(content, /从证据到方案的迭代闭环/);
